#!/usr/bin/env bash
# ============================================================
# Coturn STUN Server — VPS Setup Wizard
...
preflight() {
    banner "Coturn STUN Server — VPS Setup Wizard"

    echo -e "This wizard will install and configure coturn as a STUN server"
...
gather_info() {
    ...
    echo -e "  Mode:           ${BOLD}STUN-only${NC} (no TURN relay)"
    echo -e "  Auth:           ${BOLD}none${NC} (STUN does not require auth)"
    echo ""
...
# ============================================================
# Coturn STUN Server Configuration
...
# No authentication needed for STUN
no-auth

# No TLS needed for STUN (STUN is inherently safe)
no-tls
no-dtls
...
setup_firewall_and_service() {
    ...
    if command -v ufw &>/dev/null; then
        ...
        ok "UFW: 3478/udp ALLOW, 3478/tcp ALLOW"
    else
        warn "UFW not found — make sure port 3478 (UDP+TCP) is open in your firewall"
    fi
